...
public class ReviewTaskRecoveryOnStartup {
    ...
    public void recoverRunningTasks() {
        ...
        ensureColumn("llm_provider",
                "ALTER TABLE review_task ADD COLUMN llm_provider VARCHAR(32) DEFAULT NULL COMMENT 'llm provider used by this task'");
        ensureColumn("start_time",
                "ALTER TABLE review_task ADD COLUMN start_time DATETIME DEFAULT NULL COMMENT 'review execution start time'");
        ensureColumn("finish_time",
                "ALTER TABLE review_task ADD COLUMN finish_time DATETIME DEFAULT NULL COMMENT 'review execution finish time'");
        ensureColumn("duration_ms",
                "ALTER TABLE review_task ADD COLUMN duration_ms BIGINT DEFAULT NULL COMMENT 'review execution duration in milliseconds'");
